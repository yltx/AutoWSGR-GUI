...
function isEnabledThreshold(value: number | undefined): value is number {
...
export class StopConditionChecker {
  ...
  checkRunning(cond: StopCondition): boolean {
    let met = false;
    if (isEnabledThreshold(cond.loot_count_ge) && this.trackedLootCount != null && this.trackedLootCount >= cond.loot_count_ge) {
      this.emitLog('info', `战利品已达 ${this.trackedLootCount}/${cond.loot_count_ge}，实时触发停止`);
      met = true;
    }
    if (isEnabledThreshold(cond.ship_count_ge) && this.trackedShipCount != null && this.trackedShipCount >= cond.ship_count_ge) {
  ...
  /**
   * 掉落检查：在发起 taskStart 之前确认停止条件是否已满足。
   * 仅依赖 OCR：调用 /api/game/acquisition 读取出征面板数量。
   */
  async preflightCheck(cond: StopCondition, taskName: string): Promise<boolean> {
    Logger.debug(`[StopCond] 掉落检查: 「${taskName}」 条件=${jsonCodec.stringify(cond)}`, 'scheduler');

    if (
      !isEnabledThreshold(cond.loot_count_ge)
      && !isEnabledThreshold(cond.ship_count_ge)
    ) {
      return false;
    }
    ...
    try {
      ...
      if (resp.success && resp.data) {
        ...
        if (isEnabledThreshold(cond.loot_count_ge) && loot_count != null && loot_count >= cond.loot_count_ge) {
          this.emitLog('info', `[掉落] 战利品 ${loot_count} ≥ ${cond.loot_count_ge}，满足停止条件`);
          return true;
        }
        if (isEnabledThreshold(cond.ship_count_ge) && ship_count != null && ship_count >= cond.ship_count_ge) {
          this.emitLog('info', `[掉落] 舰船 ${ship_count} ≥ ${cond.ship_count_ge}，满足停止条件`);
          return true;
        }

        this.emitLog('info', `[掉落] 战利品=${loot_count ?? '-'} 舰船=${ship_count ?? '-'}，未达停止条件`);
      } else {
        this.emitLog('warn', `[掉落] 检查失败: ${resp.error ?? 'unknown error'}`);
      }
    } catch (e) {
      this.emitLog('warn', `[掉落] 检查异常: ${String(e)}`);
    }

    Logger.debug(`[StopCond] 掉落检查: 未满足停止条件，任务将启动`, 'scheduler');
    return false;
  }
  ...
  async checkCondition(cond: StopCondition, _taskName: string): Promise<boolean> {
    ...
    try {
      ...
      if (isEnabledThreshold(cond.loot_count_ge) && data.dropped_loot_count != null && data.dropped_loot_count >= cond.loot_count_ge) {
        this.emitLog('info', `战利品已达 ${data.dropped_loot_count}，满足停止条件 (≥${cond.loot_count_ge})`);
        return true;
      }
      if (isEnabledThreshold(cond.ship_count_ge) && data.dropped_ship_count != null && data.dropped_ship_count >= cond.ship_count_ge) {
