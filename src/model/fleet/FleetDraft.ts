...
export function hasOtherPrimaryShip(
  ...
  return slots.some((slot, position) => (
    ...
    && slot.primary?.searchName === searchName
  ));
}
...
export function resolveGalleryFormationAssignment(
  ...
  const existingPosition = slots.findIndex(
    slot => slot.primary?.searchName === searchName,
  );
...
function teamRuleConstraintsFromDraft(
  rule: FleetRuleDraft,
  field: string,
): Omit<FleetTeamShipRule, 'name' | 'searchName'> {
  ...
  const result: Omit<FleetTeamShipRule, 'name' | 'searchName'> = {};
  if (rule.shipTypes.length > 0) {
    result.shipTypes = [...rule.shipTypes];
  }
  if (rule.levelEnabled && rule.minLevel !== null) {
    result.minLevel = rule.minLevel;
  }
  if (rule.levelEnabled && rule.maxLevel !== null) {
    result.maxLevel = rule.maxLevel;
  }
  return result;
}
...
function teamShipRuleFromDraft(
  ...
  const result: FleetTeamShipRule = {
    name: ship.name,
    ...teamRuleConstraintsFromDraft(rule, field),
  };
  if (Object.keys(rule.extensions).length > 0) {
    result.extensions = { ...rule.extensions };
  }
  if (ship.searchName && ship.searchName !== ship.name) {
    result.searchName = ship.searchName;
  }
  return result;
}
...
function teamPlanSlotFromDraft(
  ...
  const result: FleetTeamPlanSlot = slot.primary
  ...
  if (Object.keys(slot.extensions).length > 0) {
    result.extensions = { ...slot.extensions };
  }
  if (backups.length > 0) {
...
export function fleetDraftToTeamPlan(
  ...
  const plan: FleetTeamPlan = {
    name,
    ships: occupiedSlots.map(teamPlanSlotFromDraft),
  };
  if (Object.keys(draft.extensions).length > 0) {
    plan.extensions = { ...draft.extensions };
  }
  return plan;
}
...
function findTeamPlanShip(
  ...
  const ship = ships.find(item => item.name === rule.name)
    ?? ships.find(item => item.searchName === rule.searchName);
...
function fleetSlotFromTeamPlan(
  ...
  return {
    ...
    shipTypes: [...(slot.shipTypes ?? [])],
    levelEnabled: (
      slot.minLevel !== undefined || slot.maxLevel !== undefined
    ),
    minLevel: slot.minLevel ?? null,
    maxLevel: slot.maxLevel ?? null,
