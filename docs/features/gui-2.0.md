...
手动安全强化继续持久化策略但不进入 Scheduler。界面可调用只读库存 Session 与
snapshot preview API，用户必须显式选择目标和素材 occurrence；预览固定不可执行，
界面不挂载强化执行入口，也不会消耗舰船。
...
切换到 `latest`，需要手动安装 2.1.0。若必须回退，应使用上述备份和旧安装器，
...
- [ ] 手动安全强化不进入 Scheduler，可生成只读 Session 预览，且不挂载后端执行入口。
- [ ] 保存接口缺失或写入失败时不显示成功。
- [ ] 舰船资料库状态和更新进度可见。
...
- [ ] `git diff --check` 通过。
- [ ] `npm run dist` 生成 `2.1.0` 安装包和 `latest.yml`。
